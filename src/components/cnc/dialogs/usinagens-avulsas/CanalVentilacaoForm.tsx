import { CanalVentilacaoConfig, calcularPosicoesCanaisVentilacao } from "./types";
...
function PecaPreview({ config }: { config: CanalVentilacaoConfig }) {
  const { largura, altura, espessuraCanal, ladoReferencia } = config;
      ...
      <g transform={`translate(${pad},${pad})`}>
        ...
        {positions.map((pos, i) => {
          if (isHorizontal) {
            ...
            return <rect key={i} x={4} y={cy} width={sw - 8} height={Math.max(channelW, 1)}
              fill="hsl(200 70% 50% / 0.3)" stroke="hsl(200 70% 45%)" strokeWidth={0.8} />;
          } else {
            const cx = pos * scale - channelW / 2;
            return <rect key={i} x={cx} y={4} width={Math.max(channelW, 1)} height={sh - 8}
              fill="hsl(200 70% 50% / 0.3)" stroke="hsl(200 70% 45%)" strokeWidth={0.8} />;
          }
        })}
